...
size_t YOLOutils::vectorProduct(const std::vector<int64_t>& vector)
{
    if (vector.empty())
        return 0;
...
std::wstring YOLOutils::charToWstring(const char* str)
{
...
std::vector<std::string> YOLOutils::loadNames(const std::string& path)
{
    // load class names
...
void YOLOutils::visualizeDetection(cv::Mat& im, std::vector<Yolov11Result>& results,
    const std::vector<std::string>& classNames)
{
    cv::Mat image = im.clone();
    for (const Yolov11Result& result : results)
    {
...
cv::Mat YOLOutils::getBinaryMask(cv::Mat& img, std::vector<Yolov11Result>& results)
{
    cv::Mat out(img.size(), CV_8UC1, cv::Scalar(255));
    for (const Yolov11Result& result : results)
    {
        out(result.box).setTo(cv::Scalar(result.classId), result.boxMask);
    }
    return out;
}

void YOLOutils::letterbox(const cv::Mat& image, cv::Mat& outImage,
...
void YOLOutils::scaleCoords(cv::Rect& coords,
    cv::Mat& mask,
